...
- `GET /recordings` — list/filter: `provider_id`, `channel_id`, `status` (`scheduled | recording | completed | failed | cancelled`), `start_after`/`start_before`, `recurring_rule_id`, `include_projected` (also return computed-but-not-yet-materialized future occurrences of recurring rules, flagged as projected). **Implemented 2026-07-19** (`server/src/routes/recordings.ts`, projection logic in `server/src/scheduler/projection.ts`). `includeProjected=true` walks each matching non-cancelled recurring rule forward day-by-day from today out to `PROJECTION_HORIZON_DAYS` (30, `src/config.ts` — bounded, since an indefinite rule has no natural stopping point), applying the same day-active/skip-exception/end-date/max-occurrences rules the scheduler tick (`scheduler/tick.ts`) uses, so a projected occurrence is exactly what the tick would eventually materialize (barring a hard-reject at fire time, which can't be known this far ahead — projections are optimistic). Already-materialized occurrences are excluded from the projected set (they're already in the real result rows) rather than double-reported; every returned row gets a `projected: true`/`false` flag so the two are "clearly distinguished," per PLAN.md's own wording above. A `status` filter for anything other than `scheduled` (or omitted) suppresses projection entirely — there's no such thing as a projected `completed`/`failed`/`cancelled` occurrence. Verified live: a 7-day-a-week rule projected the expected 31 occurrences (today + 30 days) with none set; `maxOccurrences: 5` capped it to exactly 5; an `endDate` cut it off mid-window by exact timestamp, not calendar day; skipping today's occurrence preserved the total (the walk found a replacement occurrence further out rather than shrinking the count) confirming skips don't consume max_occurrences quota; and `status=completed` correctly excluded all projected rows.
